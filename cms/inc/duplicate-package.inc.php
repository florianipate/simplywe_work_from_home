        ...
        if(Input::exists()){
            ...
            echo '<span style="color:#f00">You must select at least on day of the week!</span><br>';
                ...
                try{
                    ...
                    $package_info = DB::getInstance()->get('cms_venue_packages', array('venue_ref', '=', $venue_ref));
                    if($package_info->count()){
                        foreach($package_info->results() as $package){
                            $new_package_id = $package->id;
                        }
                        Session::put('new_package_id', $new_package_id);
                        Session::put('old_package_id', $package_id);
                    }
                    Redirect::to('duplicate-additional-package-details.php?id='.$package_id);
                } catch(Exception $e) {
                    die($e->getMessage());
                    }
                }
